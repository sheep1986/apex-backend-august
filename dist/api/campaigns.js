      ...
      `)
        ...
        const campaignsWithMetrics = await Promise.all((campaigns || []).map(async (campaign) => {
            ...
            const conversionRate = callsCount && callsCount > 0
                ...
                : 0;
            const leadGoal = campaign.lead_goal || 100;
...
router.post('/', async (req, res) => {
    try {
        const { name, description, type, industry, target_audience, script, voice_settings, schedule, budget, phone_numbers, status = 'draft', lead_goal, start_date, end_date, assistant_id, timezone, calling_hours, calling_days } = req.body;
        const userId = req.user?.id;
        const organizationId = req.user?.organizationId;
        if (!userId || !organizationId) {
            return res.status(401).json({ error: 'Unauthorized' });
