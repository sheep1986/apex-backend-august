...
interface AppointmentRequest {
  ...
  timezone?: string;
  duration?: number;
  type?: string; // demo, consultation, follow-up
  notes?: string;
}
...
export interface TranscriptAnalysis {
  ...
  leadType: 'b2b' | 'b2c'; // Business or Consumer
  ...
  consumerContext?: {
    propertyType?: string; // home, condo, etc.
    ownership?: string; // owner, renter
    household?: string; // single, family
    motivation?: string; // cost savings, environment, etc.
  };
  customFields: Record<string, any>;
}

export class AITranscriptAnalyzer {
   ...
   */
  async analyzeTranscript(callData: any, campaignContext?: any): Promise<TranscriptAnalysis> {
    try {
   ...
   */
  private formatTranscript(callData: any): string {
   ...
   */
  private buildAnalysisPrompt(transcript: string, callData: any, campaignContext?: any): string {
...
6. Extract ALL contact information mentioned, even partial`;
   ...
   * Validate and enhance the analysis
   */
  private validateAnalysis(analysis: any, callData: any, campaignContext?: any): TranscriptAnalysis {
    // Determine lead type with smart defaults
    let leadType: 'b2b' | 'b2c' = analysis.leadType || 'b2c';
    
    // Override based on campaign context if provided
    ...
    // If still not determined, use analysis clues
    if (!analysis.leadType) {
      ...
      // Default to B2C if no business indicators
      else {
        leadType = 'b2c';
      }
    }
    
    // Clean contact info based on lead type
    const contactInfo = {
    ...
    const validated: TranscriptAnalysis = {
    ...
    // Auto-enhance interest level based on indicators
  ...
  /**
   * Analyze call in real-time (for live transcription)
   */
  async analyzeRealTime(partialTranscript: string): Promise<Partial<TranscriptAnalysis>> {
...
export default AITranscriptAnalyzer;