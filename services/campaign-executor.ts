...
import { addMinutes, addHours, addDays, isWithinInterval, parseISO, format } from 'date-fns';
...
export class CampaignExecutor {
  ...
  constructor() {
    // Temporarily disabled due to date-fns-tz issue
    // this.startScheduler();
  }
  ...
  private async saveCallRecord(queuedCall: QueuedCall, vapiResult: any, outcome: string) {
    ...
    if (outcome !== 'no_answer' && outcome !== 'failed' && transcript) {
      ...
      try {
        const { processCallWithAI } = await import('./ai-call-processor');
        // Pass the full VAPI result including the transcript
        const vapiCallData = {
          ...vapiResult.call,
