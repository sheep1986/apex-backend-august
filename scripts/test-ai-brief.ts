...
/**
 * Test script to verify the new AI brief format is working
 * This uses a sample transcript to test the AI processing
 */

// Sample transcript for testing (Matt's solar consultation call)
...
async function testAIBrief() {
  ...
  try {
    // Get first existing organization and campaign for testing
    const { data: orgs } = await supabase.from('organizations').select('id').limit(1);
    const { data: campaigns } = await supabase.from('campaigns').select('id').limit(1);
    
    // Create a mock call record with proper schema
    const mockCall = {
      ...
      cost: 0.15,
      status: 'completed',
      outcome: 'interested',  // Valid outcome from the database
      created_at: new Date().toISOString()
    };
    
    // Insert the mock call into database temporarily
    ...
    // Process with the enhanced AI
    const result = await EnhancedAIProcessor.processCall(
      insertedCall.id,
      SAMPLE_TRANSCRIPT,
      null // no vapi_data for test
    );
    
    if (result) {
      console.log('✅ AI processing complete!\n');
      
      // Fetch the lead that was created/updated
      ...
      await supabase.from('calls').delete().eq('id', insertedCall.id);
...
// Alternative: Test with an existing call ID
async function testExistingCall(callId: string) {
  ...
  try {
    ...
    const result = await EnhancedAIProcessor.processCall(
      call.id,
      transcript,
      null // vapi_data might not exist
    );
