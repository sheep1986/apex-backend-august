...
/**
 * STABLE VAPI WEBHOOK HANDLER
 * 
 * This webhook handler is designed to be:
 * 1. Update-resistant - No dependencies on complex org structures
 * 2. Data-preserving - Captures ALL webhook data in raw format
 * 3. Simple - Uses direct email storage instead of org relationships
 * 4. Robust - Handles any VAPI webhook event type
 */

/**
 * POST /api/stable-vapi/webhook
 * Main stable VAPI webhook handler - captures ALL VAPI data
 */
router.post('/webhook', async (req: Request, res: Response) => {
  try {
    ...
    if (insertError) {
      ...
      // Log the error but still return success to VAPI to prevent retries
...
/**
 * Extract and normalize webhook data from payload
 */
function extractWebhookData(payload: any, sourceIp: string, userAgent: string): any {
  const call = payload.call || {};
  const assistant = payload.assistant || {};
  const phoneNumber = payload.phoneNumber || {};
  const message = payload.message || {};

  // Determine user email (this is the key for avoiding org dependencies)
  let userEmail = null;
  ...
  if (call.metadata?.userEmail) {
    ...
    // Default to test user for now - in production, this should be determined
    // by the phone number or assistant configuration
 ...
 */
async function logWebhookError(payload: any, error: any, sourceIp: string, userAgent: string): Promise<void> {
  try {
    const errorData = {
      webhook_type: payload?.type || 'unknown',
...
/**
 * GET /api/stable-vapi/data
 * Get captured VAPI data with filtering options
 */
...
/**
 * GET /api/stable-vapi/calls/:callId
 * Get all webhook data for a specific call
 */
router.get('/calls/:callId', async (req: Request, res: Response) => {
  try {
    ...
    // Build a comprehensive call summary
 ...
 */
function buildCallSummary(events: any[]): any {
