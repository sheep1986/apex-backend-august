...
// POST /api/vapi-call-webhook - Receive call completion data from VAPI
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    // Determine call outcome from VAPI data
    let outcome = 'failed';
    ...
    if (call?.endedReason) {
      switch (call.endedReason.toLowerCase()) {
        ...
        case 'assistant-ended-call':
          outcome = 'answered'; // AI completed the call
          break;
        default:
          outcome = 'failed';
      }
    ...
    // AI Analysis
    let ai_sentiment_score: number | null = null;
    let ai_qualification_score: number | null = null;
    ...
    if (analysis) {
      ...
      // Determine if qualified based on AI analysis
      is_qualified = (
        ai_qualification_score && ai_qualification_score > 0.7
      ) || (
        analysis.qualified === true
      ) || (
        ai_summary && ai_summary.toLowerCase().includes('interested')
      ) || false;
    ...
    // Calculate call cost (rough estimate based on duration)
    const call_cost_usd = (callData.duration_seconds / 60) * 0.075; // ~$0.075 per minute

    // Try to find the campaign from metadata or phone number
    let campaign_id: string | null = null;
    let organization_id: string | null = null;
    
    // Look for campaign info in VAPI metadata
    if (call?.metadata?.campaignId) {
      campaign_id = call.metadata.campaignId;
    } else if (call?.assistantId) {
      // Try to find campaign by assistant ID (you might need to adjust this logic)
    ...
    // If we can't find campaign, try to match by phone number pattern
    if (!campaign_id) {
      console.log('⚠️ No campaign found for call, creating generic entry');
      // You might want to create a "default" campaign for unmatched calls
    ...
    // Get organization_id if not found
    ...
    // If qualified, create or update contact in CRM
      ...
      try {
        ...
        if (existingContact) {
          ...
          if (contactError) {
            ...
            // Update call attempt to link to contact
            await supabase
              .from('call_attempts')
              .update({ 
        ...
        // Don't fail the webhook for contact creation errors
      }
    }
...
// GET /api/vapi-call-webhook/test - Test endpoint for webhook
